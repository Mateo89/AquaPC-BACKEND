...
def process_times_point(times):
    ...
    time_on = time_now.replace(hour=time_on_split[0], minute=time_on_split, second=0, microsecond=0)
...
def process_times_between(times):
    ...
    time_off = time_now.replace(hour=time_off_split[0], minute=time_off_split[1], second=0,
                                microsecond=0)
    time_on = time_now.replace(hour=time_on_split[0], minute=time_on_split, second=0, microsecond=0)
...
# pobranie stanu jednego wczesniej przed time_now i jednego pozniej po time_now
