...
class Light1SettingWindow():
    ...
    def __init__(self):
        ...
        self.barTile = BarTile(Coordinate(0, 0, 400, 25), "Lampa 1", "settings25.png")
        ...
        self.on_hour = Register.LIGHT1_ON_HOUR
        self.on_min = Register.LIGHT1_ON_MINUTE

        self.off_hour = Register.LIGHT1_OFF_HOUR
        self.off_min = Register.LIGHT1_OFF_MINUTE
        self.use_lamp = Register.LIGHT1_TEMP_USE_DELTA
        self.change_water = Register.LIGHT1_CHANGE_WATER_ON_FLAG
    ...
    def event(self):
        ...
        if lirc_event == LircEvents.KEY_RIGHT:
            self.selectedIndex = (self.selectedIndex + 1) % 13

        if lirc_event == LircEvents.KEY_LEFT:
            self.selectedIndex = (self.selectedIndex - 1) % 13

        if lirc_event == LircEvents.KEY_UP:
            self.selectedIndex = (self.selectedIndex - 4) % 13
        ...
        if lirc_event == LircEvents.KEY_OK:
            ...
            if self.selectedIndex == 9:
                self.use_lamp -= 0.1
            if self.selectedIndex == 10:  #change_water flag
                self.change_water = not self.change_water
            if self.selectedIndex == 11:  #reset
                self.reset()
            if self.selectedIndex == 12:  #zapisz
                self.save()
                self.reset()
    ...
    def save(self):
        Register.LIGHT1_ON_HOUR = self.on_hour
        Register.LIGHT1_ON_MINUTE = self.on_min
        Register.LIGHT1_OFF_HOUR = self.off_hour
        Register.LIGHT1_OFF_MINUTE = self.off_min
        Register.LIGHT1_TEMP_USE_DELTA = self.use_lamp
        Register.LIGHT1_CHANGE_WATER_ON_FLAG = self.change_water
    ...
    def draw(self):
        ...
        if Register.LIGHT1_ON_HOUR != self.on_hour:
        ...
        cord = Coordinate(*main_cord.get_slice(30))
        if Register.LIGHT1_ON_MINUTE != self.on_min:
            pygame.draw.rect(self.lcd, Colors.RED, pygame.Rect(cord.locate(0, 0), cord.size()))
        ...
        cord = Coordinate(*main_cord.get_slice(30))
        if Register.LIGHT1_OFF_HOUR != self.off_hour:
            pygame.draw.rect(self.lcd, Colors.RED, pygame.Rect(cord.locate(0, 0), cord.size()))
        ...
        cord = Coordinate(*main_cord.get_slice(30))
        if Register.LIGHT1_OFF_MINUTE != self.off_min:
            pygame.draw.rect(self.lcd, Colors.RED, pygame.Rect(cord.locate(0, 0), cord.size()))
        ...
        main_cord = Coordinate(0, 110, 400, 30)
        cord = Coordinate(*main_cord.get_slice(270))
        text_surface = Fonts.font_40.render("GRZ. TEMP DELTA",
                                            True, Colors.WHITE)
        self.lcd.blit(text_surface, cord.get_center(text_surface.get_width(), text_surface.get_height()))

        cord = Coordinate(*main_cord.get_slice(50))
        if Register.LIGHT1_TEMP_USE_DELTA != self.use_lamp:
            pygame.draw.rect(self.lcd, Colors.RED, pygame.Rect(cord.locate(0, 0), cord.size()))
        text_surface = Fonts.font_40.render(str(self.use_lamp),
                                            True, Colors.WHITE)
        self.lcd.blit(text_surface, cord.get_center(text_surface.get_width(), text_surface.get_height()))
        ...
        if Register.LIGHT1_CHANGE_WATER_ON_FLAG != self.change_water:
